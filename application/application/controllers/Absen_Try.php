...
class Absen extends CI_Controller {
  ...
  public function convertdate($EXCEL_DATE)
  {
    ...
    $temp = strtotime(PHPEXcel_Style_NumberFormat::toFormattedString($EXCEL_DATE,'YYYY-mm-dd'));
    $actualdate = date('Y-m-d',$temp);
    return $actualdate; 
  }

   public function converttime($EXCEL_TIME)
  {

    $temp = strtotime(PHPEXcel_Style_NumberFormat::toFormattedString($EXCEL_TIME,'H:i:s'));
    $actualtime = time ('H:i:s',$temp);
    return $actualtime; 
  }
  ...
  if (!$this->upload->do_upload('file')) 
  {
   ...
   for ($row = 2; $row <= $highestRow; $row++){  
     ...
     "late"=> $this->converttime($rowData[0][13]),
